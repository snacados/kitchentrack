...
// ─── Storage ───
const STORAGE_KEY = "kitchen_inventory_v1";
    ...
    <div className={`item-card ${status} ${compact ? "compact" : ""}`}>
      ...
      <div className="item-card-actions">
        <QuantityControl
          ...
          onChange={(q) => onUpdate(item.id, { quantity: q })}
        />
...
function AddItemForm({ onAdd, initialLocation = "fridge" }) {
  ...
  useEffect(() => {
    setCategory(LOCATION_CATEGORIES[location][0]);
  }, [location]);
  ...
  const subtypeOptions = EXPIRATION_RULES[location]?.[category]
    ? Object.keys(EXPIRATION_RULES[location][category])
    : ["other"];
...
function DraggableScroll({ children, className }) {
  ...
  const onPointerUp = (e) => {
    ...
    el.style.userSelect = "";
    // Suppress click on child buttons if we dragged
    if (hasMoved.current) {
...
export default function KitchenInventory() {
  ...
  const transferMeat = useCallback((item) => {
  ...
  const stats = useMemo(() => {
    ...
    const expired = active.filter(i => getDaysUntilExpiry(i) <= 0);
    return {
      total: active.length,
            ...
            {search ? (
              ...
              <>
                <div className="stats-row">
                  {[
                    { icon: "❄️", num: stats.byLocation.fridge, label: "Fridge", page: "fridge" },
                    { icon: "🧊", num: stats.byLocation.freezer, label: "Freezer", page: "freezer" },
                    { icon: "🏠", num: stats.byLocation.pantry, label: "Pantry", page: "pantry" },
                    { icon: "🌶️", num: stats.byLocation.spices, label: "Spices", page: "spices" },
                  ].map(s => (
                    <div key={s.page} className="stat-card" onClick={() => setPage(s.page)}>
      ...
      <nav className="bottom-nav">
        <div className="nav-inner">
          {navItems.map(nav => (
            <button
              key={nav.id}
              className={`nav-btn ${page === nav.id ? "active" : ""}`}
              onClick={() => { setPage(nav.id); setSearch(""); }}
            >
              <span className="nav-icon">{nav.icon}</span>
              <span className="nav-label">{nav.label}</span>
              {nav.id === "home" && stats.expiring > 0 && (
                <span className="nav-badge">{stats.expiring}</span>
              )}
            </button>
          ))}
        </div>
